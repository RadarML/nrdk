...
from .transforms import RealSpectrum
...
@dataclass
class Occupancy2D(Generic[TArray]):
    """2D occupancy data.
...
class LidarOccupancy3D:
    ...
    def __call__(
        ...
        return Occupancy3D(occupancy=res, timestamps=lidar.timestamps)


class LidarOccupancy2D:
    ...
    def __call__(
        ...
        res[:, :, :, 0] = False
        return Occupancy2D(occupancy=res, timestamps=lidar.timestamps)
