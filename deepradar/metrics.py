...
class L1Hausdorff:
    """L1 Hausdorff distance for a 2D occupancy grid."""

    @staticmethod
    def as_distance(mask: Bool[Tensor, "b w h"]) -> Integer[Tensor, "b w h"]:
    ...
    def __call__(
        ...
        """Compute grid hausdorff distance, in grid units.
        
        NOTE: the distance is always an integer.
        """
        d1 = torch.max(self.as_distance(y_true) * y_hat, dim=(1, 2)).values
        d2 = torch.max(self.as_distance(y_hat) * y_true, dim=(1, 2)).values
        return torch.maximum(d1, d2)


class PolarHausdorff:
    """L2 Hausdorff distance for a polar range-azimuth grid."""
    ...
    def __call__(
        ...
        """Compute hausdorff distance, in range bins."""
        ...
        def _forward(x, y):
            ...
            return torch.maximum(
                torch.max(torch.min(dist, dim=0).values),
                torch.max(torch.min(dist, dim=1).values))

        return torch.stack([_forward(xs, ys) for xs, ys in zip(y_hat, y_true)])
...
class CombinedDiceBCE:
    ...
    def __call__(
        ...
        bce = binary_cross_entropy(y_hat, y_true, reduction='none')
