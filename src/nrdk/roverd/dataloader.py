...
def datamodule(
    ...
    When only a `train` trace list is provided, it is split into train and
    validation portions using [`roverd.split`][roverd.split]. When an explicit
    `val` trace list is provided, both lists are used in full instead.
    ...
    Args:
        ...
        ptrain: proportion of the data to use for the training split; takes
            the first `ptrain` of each trace when no explicit `val` trace list
            is provided.
        pval: proportion of the data to use for the validation split; takes the
            last `pval` of each trace when no explicit `val` trace list is
            provided.
    ...
    def split_closure(
        name: str, trace_paths: Sequence[str], train: bool = False
    ) -> Callable[[], DatasetMeta]:
        def closed():
            return DatasetMeta(
                dataset(trace_paths), meta={"train": train, "split": name}
            )

        return closed

    splits = {
        name: split_closure(name, trace_paths)
        for name, trace_paths in traces.items()
        if name != "train"
    }

    if "train" in traces:
        if "val" in traces:
            splits["train"] = split_closure(
                "train", traces["train"], train=True
            )
        else:
