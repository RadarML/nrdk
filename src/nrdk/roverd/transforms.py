...
@dataclass
class VelocityData(Generic[TArray]):
...
class Spectrum(Transform[
    types.XWRRadarIQ[np.ndarray], SpectrumData[np.ndarray]
]):
...
class Velocity(Transform[
    types.Pose[np.ndarray], VelocityData[np.ndarray]
]):
    ...
    def __call__(
        ...
        radar: SpectrumData[np.ndarray] | None = None,
        aug: Mapping[str, Any] = {}
    ) -> VelocityData[np.ndarray]:
        ...
        return VelocityData(vel=vel, timestamps=pose.timestamps)
